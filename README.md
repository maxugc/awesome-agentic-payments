...
- [stipend](https://github.com/stipend-sh/stipend) — non-custodial USDC wallet on Base that an AI agent installs by itself; per-transaction, per-day and per-counterparty caps plus a destination allowlist are enforced in code between the decision and the signature rather than in a prompt, and apply identically to buyer-side x402 auto-payments. Local stdio MCP server with 7 tools. Python, Apache-2.0, unaudited.
- [Animica x402 APIs](https://animica.dev/x402) — 47 pay-per-request x402 services for agents (web research, extraction, verifiable randomness, chain data, GEO audit) and an agent-to-agent job network with escrow; settles USDC on Base through a self-hosted facilitator or natively in ANM, with prepaid credits and a free trial per product.
